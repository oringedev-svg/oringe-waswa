...
export default function GlobalSearch() {
  ...
  // Below sm the bar itself is hidden (an always-visible text input has no
  // room next to theme/notifications/sign-out at 375px) and replaced by an
  // icon trigger that opens this as a full-width overlay instead.
        ...
        <div className="py-1">
          {results.map((r, i) => (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 flex items-center justify-between gap-2 transition-colors min-h-11 ${
  ...
  return (
    <div ref={containerRef} className="relative flex-shrink-0">
      {/* Desktop/tablet: inline bar, unchanged. */}
      <div className="relative w-full max-w-xs hidden sm:block">
        ...
        />
        <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[0.65rem] px-1.5 py-0.5 rounded border border-[color-mix(in_srgb,var(--on-band)_20%,transparent)] text-[var(--on-band-muted)] hidden lg:block">
          ⌘K
        </kbd>
      ...
      {/* Mobile: icon-only trigger, same 44px target as the other topbar
          icons, opens a full-width overlay instead of squeezing a text
          input in next to theme/bell/sign-out. */}
      <button
        onClick={() => { setMobileOpen(true); setTimeout(() => mobileInputRef.current?.focus(), 0) }}
        aria-label="Search"
        className="sm:hidden inline-flex items-center justify-center min-h-11 min-w-11 rounded-md text-[var(--on-band-muted)] hover:bg-[color-mix(in_srgb,var(--on-band)_8%,transparent)] hover:text-[var(--on-band)] transition-colors"
      >
        <Search className="w-4 h-4" />
      </button>

      {mobileOpen && (
        <div className="sm:hidden fixed inset-0 z-[60] bg-[var(--color-surface)] flex flex-col">
          <div className="flex items-center gap-2 p-3 border-b border-[var(--color-border)]">
            <div className="relative flex-1">
              ...
              <input
                ...
                onKeyDown={handleKeyDown}
                placeholder="Search everything…"
                className="w-full pl-9 pr-3 text-base py-2.5 bg-[var(--color-surface-raised)] border border-[var(--color-border)] text-[var(--color-text-primary)] placeholder:text-[var(--color-muted)] rounded-[var(--radius-md)] focus:outline-none focus:border-[var(--color-brand)]"
              />
            </div>
            <button
              onClick={() => { setMobileOpen(false); setQuery(''); setResults([]) }}
              aria-label="Close search"
              className="inline-flex items-center justify-center min-h-11 min-w-11 rounded-md text-[var(--color-text-muted)] hover:bg-[var(--color-surface-overlay)] flex-shrink-0"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
