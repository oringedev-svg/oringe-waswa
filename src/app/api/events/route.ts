...
export async function POST(req: NextRequest) {
  const guard = await requirePermissionApi('manage_events')
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'events', record_id: data.id, action: 'INSERT', new_data: body })
...
export async function PATCH(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'events', record_id: id, action: 'UPDATE', new_data: updates })
  return NextResponse.json(data)
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'events', record_id: id, action: 'DELETE' })
