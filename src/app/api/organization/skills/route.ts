...
export async function POST(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'skills', record_id: data.id, action: 'INSERT', new_data: body })
...
export async function PATCH(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'skills', record_id: id, action: 'UPDATE', new_data: updates })
  return NextResponse.json(data)
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('skills').update({ archived_at: new Date().toISOString() }).eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  await logAudit({ table_name: 'skills', record_id: id, action: 'DELETE' })
