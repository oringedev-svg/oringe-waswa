...
import { Plus, Trash2, Edit, Star, Loader2, X, Archive, Download, ArchiveRestore } from 'lucide-react'
...
export default function AdminAwardsPage() {
  ...
  const selection = useSelection(awards)
  ...
  async function save() {
    ...
    try {
      const res = isNew
        ...
        : await fetch('/api/awards', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editing) })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-semibold text-[var(--color-text-primary)]">Awards</h1>
          <p className="text-[var(--color-text-muted)] text-sm mt-1">{awards.length} awards {trash ? 'in trash' : ''}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center text-[var(--color-muted)]">No awards yet.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {awards.map(award => (
            <div key={award.id} className="card p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-start gap-2">
                  <input type="checkbox" className="mt-1" checked={selection.selected.has(award.id)} onChange={() => selection.toggle(award.id)} />
                  <div>
                    <h3 className="font-semibold text-sm text-[var(--color-text-primary)] flex items-center gap-1.5">
                      {award.is_featured && <Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />}
                      {award.title}
                    </h3>
                    <p className="text-xs text-[var(--color-muted)]">{award.issuer}{award.year ? ` · ${award.year}` : ''}</p>
                  </div>
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  {trash ? (
                    <button onClick={async () => { await fetch('/api/awards', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: award.id, restore: true }) }); load() }}
                      className="btn btn-ghost p-1.5 !px-1.5 text-[var(--color-accent)]"><ArchiveRestore className="w-3.5 h-3.5" /></button>
                  ) : (
                    <>
                      <button onClick={() => { setEditing(award); setIsNew(false) }} className="btn btn-ghost p-1.5 !px-1.5"><Edit className="w-3.5 h-3.5" /></button>
                      <button onClick={() => deleteAward(award.id)} className="btn btn-ghost p-1.5 !px-1.5 text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                    </>
                  )}
                </div>
              </div>
              {award.description && <p className="text-xs text-[var(--color-text-secondary)] line-clamp-2">{award.description}</p>}
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-semibold text-lg text-[var(--color-text-primary)]">{isNew ? 'Add Award' : 'Edit Award'}</h2>
